...
export const ReportLostPage: React.FC = () => {
  ...
  const handleNext = () => {
    ...
    if (step === 1) {
      ...
      if (!location.trim()) {
        setError('Please enter the general area where it was lost.');
        return;
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await itemsApi.createItem({
        ...
        extraField1: lastSeenLocation.trim(),
        extraField2: reward.trim(),
  ...
  return (
    <div className="max-w-2xl mx-auto py-8 px-4 space-y-6">
      ...
      <div className="text-center space-y-1">
        <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">Report a Lost Item</h1>
        <p className="text-slate-500 text-sm">
          Follow the clean 4-step wizard to list your missing item on campus.
        </p>
      </div>
      ...
      {success ? (
        <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-md text-center space-y-4">
          ...
          <h2 className="text-2xl font-bold text-slate-900">✓ Lost item reported successfully</h2>
          <p className="text-slate-600 text-sm max-w-md mx-auto">
            Record ID: <span className="font-mono font-bold text-slate-900">{createdItemId}</span>. We'll automatically look for potential matches and notify you when something relevant is found.
          </p>
        ...
        <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-md space-y-6">
            ...
            <div className="space-y-4 animate-fade-in">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Dark Blue Nike Laptop Backpack"
                  className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-rose-600"
                />
              </div>
            ...
            <div className="space-y-4 animate-fade-in">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Describe color, brand, contents inside, unique scratches or stickers..."
                  className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-rose-600"
                />
              </div>
          ...
          {step === 3 && (
            <div className="space-y-4 animate-fade-in">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">General Campus Location *</label>
                <input
                  type="text"
                  required
                  value={location}
                  onChange={(e) => setLocation(e.target.value)}
                  placeholder="e.g. Central Library, Science Block B, Canteen"
                  className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-rose-600"
                />
              </div>

              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Last Seen Spot (More Detailed)</label>
                <input
                  type="text"
                  value={lastSeenLocation}
                  onChange={(e) => setLastSeenLocation(e.target.value)}
                  placeholder="e.g. Reading Hall Row 3, Desk 12"
                  className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-rose-600"
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Reward Offered (Optional)</label>
                  <input
                    type="text"
                    value={reward}
          ...
          {step === 4 && (
            <div className="space-y-4 animate-fade-in">
              ...
              <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-3 text-sm">
                ...
                <div className="flex justify-between border-b border-slate-200/60 pb-2">
                  <span className="text-slate-500">General Location:</span>
                  <span className="font-semibold text-slate-800">{location}</span>
                </div>
                <div className="flex justify-between border-b border-slate-200/60 pb-2">
                  <span className="text-slate-500">Last Seen Spot:</span>
                  <span className="font-semibold text-slate-800">{lastSeenLocation || 'N/A'}</span>
                </div>
                <div className="flex justify-between border-b border-slate-200/60 pb-2">
                  <span className="text-slate-500">Date Lost:</span>
                  <span className="font-semibold text-slate-800">{reportedDate}</span>
                </div>
                <div className="flex justify-between border-b border-slate-200/60 pb-2">
                  <span className="text-slate-500">Reward Offered:</span>
                  <span className="font-semibold text-amber-700">{reward || 'None'}</span>
                </div>
                <div>
          ...
          <div className="flex items-center justify-between pt-4 border-t border-slate-100">
            ...
            {step < 4 ? (
              <button type="button" onClick={handleNext} className="btn-lost text-sm shadow-sm">
              ...
              <button
                ...
                disabled={loading}
                className="btn-lost text-sm shadow-md font-semibold px-6"
              >
